...
The next step is to compute a average dynamic time warping distance between joint position traces. This means we are performing a dependent DTW, where x,y,z time series are submitted for a gesture i and gesture event j, and we perform this procedure for each joint separately (this is called an independent DTW; so we are combining an indepenedent and dependent version).
