...
We can also use a different kind of smoothing filter that where the cut-off frequency is less strictly defined such as a running average, such that sudden changes in the time series are smoothed out by relating them to adjacent data points. Below we use the butterworth filter (1st order, zero-lag, 30Hz cutoff). Note that the order becomes effectively 2nd order because we are performing the smooothing forwards and backwards.
...
ggplot(mag_D, aes(x=maxs, y = maxenv, color = g_type))+ geom_point()+geom_smooth(method = "lm")+theme_bw()+facet_wrap(.~g_type, scales ="free")+
